...
import loadedData from "../16SData2.csv";
 ...
 */
const render = async (layer, svg, legend_svg, csvData, checkbox, value) => {
  let valueIndex;

  loadedData[0].forEach((element, index) => {
    if (element == value) {
      valueIndex = index;
    }
  });
  const parsedData = loadedData.map((element) => {
  ...
  drawCirclePacking({ csvData, svg, layer, checkbox, legend_svg, value,loadedData });
};
...
const generateVisualization = () => {
  ...
  fillDataValueSelectionBox(loadedData[0].slice(1), value);
